...
def create(name='', surname='', number='', comment=''):
    global global_id
    global db
    global db_file_name
    if(name == ''):
    ...
    for row in db:
        if(row[1] == name.title() and row[2] == surname.title() and row[3] == number and row[4] == comment.lower()):
            print("already exist")
            return

    global_id += 1
    new_row = [str(global_id), name.title(),
               surname.title(), number, comment.lower()]
    db.append(new_row)
...
def retrive(id='', name='', surname='', number='', comment=''):
    global global_id
    global db
    global db_file_name
    result = []
...
def update(id='', new_name='', new_surname='', new_number='', new_comment=''):
    global global_id
    global db
    global db_file_name
    if(id == ''):
